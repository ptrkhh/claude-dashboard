...
// Claude subscription limits (the `claude /usage` bars), refreshed in the
// background so a 4s poll never waits on the network. First poll returns null;
// a transient failure keeps the last good value rather than blanking the tiles.
const USAGE_TTL_MS = 60_000;
let usageCache = { data: null, ts: 0, busy: false };
function claudeUsage(claudeDir, now) {
  if (!usageCache.busy && now - usageCache.ts > USAGE_TTL_MS) {
    usageCache.busy = true;
    fetchUsage(claudeDir)
      .then(data => { usageCache = { data: data ?? usageCache.data, ts: Date.now(), busy: false }; })
      .catch(() => { usageCache = { ...usageCache, ts: Date.now(), busy: false }; });
  }
...
export async function collectSessions(ctx) {
  ...
  return { running, resumable, stats: { ...machineStats(), disks: parseDf(dfOut), claudeUsage: claudeUsage(ctx.claudeDir, now) } };
}
